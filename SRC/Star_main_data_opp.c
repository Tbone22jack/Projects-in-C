...
void fourth_order_RungeKutta_2(double I_A, double I_M, double I_S,double delta_t,int iteration,FILE *f);
...
int main() {
	...
	delta_t=.01;
	//This puts a file down to give the final numbers of these convergent states
	FILE *fp = fopen("convergent.csv","w");
	fprintf(fp,"Stellar,Molecular,Atomic,N\n");
...
void fourth_order_RungeKutta_2(double I_A, double I_M, double I_S,double delta_t,int num,FILE *f) {
	double S_active = I_S, M_cloud = I_M, A_cloud = I_A, SS_active, SM_cloud,
...
for(double loop=delta_t;loop<MAX;loop+=delta_t)
{
	...
	if(fabs((AP_cloud-A_cloud))/A_cloud < .0001)
		isboring++;
	else
		isboring=0;
	if( isboring>100000 || A_cloud != A_cloud)
	{
		fprintf(f,"%lf,%lf,%lf,%lf\n",S_active,M_cloud,AP_cloud,N);
		break;
	}
